...
class DPMViewController: ViewController {
    ...
    func drawDetections(result: ScanResult) {
        ...
        for detection in result.getDetections() {
...
struct HostedDPMViewController: UIViewControllerRepresentable {
    @Binding var aspectRatio: Double
    func makeUIViewController(context: Context) -> DPMViewController {
        return DPMViewController(aspectRatio: $aspectRatio)
    }

    func updateUIViewController(_ uiViewController: DPMViewController, context: Context) {}
}

#Preview {
    HostedDPMViewController(aspectRatio: .constant(1.0))
}
